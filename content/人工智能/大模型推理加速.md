...
## 单层 Transfomer Block，设定：
- batch size B = 1
- 序列长度为 L
- 模型隐藏维度 d
- 注意力头数 h， 每个头维度 `d_head= d / h`

输入矩阵：

$$
X\in \mathbb{R}^{B\times L \times d} = \mathbb{R}^{1\times L \times d}
$$

## 标准 Selft-Attention 公式（无 Cache）
...
$$
Q, K,V\in \mathbb{R}^{1\times L \times d}
$$

每个头注意力：

$$
head_i=Attention(Q_i,K_i,V_i)_i=softmax({Q_iK_i^T\over \sqrt{d_{head}}})V_i \in \mathbb{R}^{1\times L \times d_{head}}
$$

- $Q_i,K_i,V_i$：是第 i 个头的 Q，K，V

最后拼接，在输出投影

$$
 Concat(head_1, \dots, head_h)W_O \in \mathbb{R}^{1\times L \times (h\cdot d_{head})} = \math
$$