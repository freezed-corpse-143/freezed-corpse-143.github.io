...
┌────────────────────────────────────────────────────────────┐
│  模型上下文窗口（短期工作记忆）                              │
│  系统指令 + AGENTS.md/skills + 历史消息 + 工具结果 + 记忆注入  │
│  超限 → 压缩（inline auto-compact / remote compact / 摘要）  │
└────────────────────────────────────────────────────────────┘
                │ 写入                        ↑ 注入
                ▼                             │
┌────────────────────────────┐   ┌────────────────────────────┐
│ 情景记忆（持久化）          │   │ 语义记忆（长期）            │
│ ~/.codex/sessions/*.jsonl │   │ ~/.codex/memories/          │
│ state_5.sqlite 等 6 个库   │──►│ memory_summary.md          │
│ ThreadStore / LiveThread  │   │ MEMORY.md / skills/         │
└────────────────────────────┘   │ rollout_summaries/        │
        │ Phase 1 提取            │（Phase 2 合并子代理更新）  │
        └─────────────────────────┘
```
